...
export default function Navbar({ activeSection, theme, themeLabel, onSectionChange, onToggleTheme }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-4 z-50 px-4">
      <nav className="glass mx-auto flex h-16 max-w-6xl items-center justify-between rounded-full px-4">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <a
              ...
              className={`focus-ring rounded-full px-3 py-2 text-xs font-semibold text-white/72 transition hover:text-white light:text-[#24344d] xl:px-4 xl:text-sm ${
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="focus-ring grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-white/10 text-xl text-white transition hover:border-cyan/60 hover:text-cyan light:border-[#091223]/15 light:bg-white/70 light:text-[#091223] lg:hidden"
          >
            {open ? <FiX /> : <FiMenu />}
          </button>
        </div>
      ...
      {open && (
        <div className="glass mx-auto mt-3 grid max-w-6xl gap-1 rounded-[1.6rem] p-3 lg:hidden">
          {navItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
